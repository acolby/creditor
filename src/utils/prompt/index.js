...
inquirer.registerPrompt('autocomplete', require('#src/utils/inquiererAutoComplete/index.js'));

async function utils_prompt(toPrompt = 'xxxxx', params) {
  const { answers, prompts, analysis } = params;
  const prompter = prompts[toPrompt]; 

  await inquirer.prompt(prompter(params))
    .then((_answers) => {
      answers[toPrompt] = _answers[toPrompt];
    })
  const nextToPrompt = prompter(params).nextPrompt && prompter(params).nextPrompt() || '';
  if (nextToPrompt) {
